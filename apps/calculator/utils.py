...
def analyze_image(img: Image, dict_of_vars: dict):
    ...
    prompt = (
        ...
        f"4. Analyzing Graphical Math problems, which are word problems represented in drawing form, such as cars colliding, trigonometric problems, problems on the Pythagorean theorem, adding runs from a cricket wagon wheel, etc. "
        f"These will have a drawing representing some scenario and accompanying information with the image. PAY CLOSE ATTENTION TO DIFFERENT COLORS FOR THESE PROBLEMS. "
        f"You need to return the answer in the format of a LIST OF ONE DICT [{{'expr': given expression, 'result': calculated answer}}]. "
        f"5. Detecting Abstract Concepts that a drawing might show, such as love, hate, jealousy, patriotism, or a historic reference to war, invention, discovery, quote, etc. "
        f"USE THE SAME FORMAT AS OTHERS TO RETURN THE ANSWER, where 'expr' will be the explanation of the drawing, and 'result' will be the abstract concept. "
        ...
        f"PROPERLY QUOTE THE KEYS AND VALUES IN THE DICTIONARY FOR EASIER PARSING WITH Python's json.loads."
    )
    ...
    # Clean response: Remove markdown (` ```json `) and extra spaces
    cleaned_text = re.sub(r'```json|```', '', response.text).strip()

    # Fix single quotes: Convert them to double quotes for JSON parsing
    cleaned_text = cleaned_text.replace("'", '"')

    try:
        answers = json.loads(cleaned_text)  # Convert cleaned response into a valid JSON
    ...
    # Ensure 'assign' key is present in every response
